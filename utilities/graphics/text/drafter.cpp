	...
	namespace graphics {
		namespace gui {
			namespace text {
				...
				rects::xywh<int> drafter::line::get_rect() const {
					return rects::ltrb<int>(0, top, right, bottom());
				}
				...
				rects::wh<int> drafter::get_bbox() const {
					if (sectors.empty() || lines.empty()) return rects::wh<int>(0, 0);
					/* plus 1 for caret, so the view caret is not canceled by align_scroll */
					return rects::wh<int>(max_x + 1, lines[lines.size() - 1].bottom());
				}

				std::pair<int, int> drafter::get_line_visibility(const rects::ltrb<int>& clipper) const {
					if (!clipper.good() || !clipper.hover(rects::xywh<int>(0, 0, get_bbox().w, get_bbox().h)))
