...
void set_color(stylesheet& subject, std::string attr, augs::rgba rgba) {
...
void set_border(stylesheet& subject, std::string attr, int w, augs::rgba rgba) {
	auto* s = resolve_attr(subject, attr);
...
void hypersomnia_gui::bind(augs::lua_state_wrapper& wrapper) {
	callback_textbox a;
	invokem(a.textbox_object.styles, "aaa", augs::rgba(1, 1, 1, 1));

	//setter_wrapper<decltype(set_color)>::invoke(a, "aaa", augs::rgba(1, 1, 1, 1));
	(a, "aaa", augs::rgba(1, 1, 1, 1));
