...
namespace db {
	namespace graphics {
		...
		namespace gui {
			font_in::font_in() : inited(false), library(new FT_Library) {
			}
			...
			bool font_in::init() {
				inited = true;
				int f = 1;
				errsf(!FT_Init_FreeType(library.get()), "freetype initialization", f);
				return f != 0;
			}
			...
			bool font_in::deinit() {
				...
				inited = false;
				return errs(!FT_Done_FreeType(*library.get()), "freetype deinitialization") == 0;
			}
			...
			font_file::charset font_file::to_charset(const std::wstring& str) {
				...
				for(auto it = s.begin(), end = s.end(); it != end; ++it)
					ranges.push_back(pair<wchar_t, wchar_t>(*it, (*it)+1));
			...
			bool font_file::open(font_in& fin, const char* filename, unsigned _pt, const charset& ranges) {
				pt = _pt;
				FT_Face face;
				int f = 1, error = FT_New_Face(*fin.library.get(), filename, 0, &face);
