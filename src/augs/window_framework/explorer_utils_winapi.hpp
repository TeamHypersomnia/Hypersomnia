...
std::optional<path_type> window::open_file_dialog(
    ...
    ofn.hwndOwner = platform->get_hwnd();
    ofn.lpstrFile[0] = '\0';
    ...
    auto show_after = scope_guard([this]() {
		ShowWindow(platform->get_hwnd(), SW_SHOW);
    });
...
message_box_button window::retry_cancel(
    ...
    return to_message_box_button(MessageBox(platform->get_hwnd(), wide_text.c_str(), wide_caption.c_str(), MB_RETRYCANCEL | MB_ICONEXCLAMATION));
}
