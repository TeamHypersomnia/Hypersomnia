...
void determinism_test_setup::process(
	...
	while (!should_quit) {
		...
		while (steps--) {
			...
			augs::write(first_cosm_reserver, hypersomnias[0].solvable.);
			first_cosm.reserve(first_cosm_reserver.get_write_pos());
			first_cosm.reset_write_pos();
			augs::write(first_cosm, hypersomnias[0].solvable.);
			...
			for (unsigned i = 1; i < cosmoi_count; ++i) {
				...
				augs::write(second_cosm_reserver, hypersomnias[i].solvable.);
				second_cosm.reserve(second_cosm_reserver.get_write_pos());
				second_cosm.reset_write_pos();
				augs::write(second_cosm, hypersomnias[i].solvable.);
