...
bool edit_property(
	...
	if constexpr(H::template handles<T>) {
		...
		if constexpr(std::is_same_v<float, T>) {
			if (label == "Opacity" || label == "Constant" || label == "Linear" || label == "Quadratic" || label == "Shadow strength" || label == "Shadow hue preservation" || label == "Point light hue preservation" || label == "Shadow smoothness") {
...
SINGLE_EDIT_FUNCTION(editor_arena_settings& insp, const editor_arena_settings defaults, id_widget_handler special_handler) {
	...
	PROPERTY("Default zoom", default_zoom);
