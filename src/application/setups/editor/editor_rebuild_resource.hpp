...
void allocate_flavours_and_assets_for_resource(
	...
	else if constexpr(std::is_same_v<editor_touch_collectible_resource, R>) {
		ensure(false && "not implemented");
	}
			...
			if (n_frames > 1) {
				...
				for (int i = 0; i < n_frames; ++i) {
					...
					*/
					...
					new_definition.source_image.path = resolve_path(resource.external_file.path_in_project);
