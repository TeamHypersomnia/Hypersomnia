	...
	*/

	{
		...
		*/
		...
		if (!loaded_existing) {
			/*
				Generate actual navmesh from the physics world.
			*/

			cosmos_navmesh new_navmesh;

			const auto cell_size = std::max(16, project.settings.navmesh_cell_size);

			/* 
				Collect islands from NAV_ISLAND markers, or fall back to AABB of all PHYSICAL sprites.
			*/

			std::vector<ltrbi> island_bounds;

			/* Check for NAV_ISLAND area markers */
			project.nodes.template get_pool_for<editor_area_marker_node>().for_each_id_and_object(
				[&](const auto&, const editor_area_marker_node& node) {
					if (!node.active) {
						return;
					}

					if (const auto res = find_resource(node.resource_id)) {
						if (res->editable.type == area_marker_type::NAV_ISLAND) {
							const auto pos = node.editable.pos;
							const auto size = node.editable.size;

							/* Round to integers for the bounds */
							const auto l = static_cast<int>(pos.x - size.x / 2.0f);
							const auto t = static_cast<int>(pos.y - size.y / 2.0f);
							const auto r = static_cast<int>(pos.x + size.x / 2.0f);
							const auto b = static_cast<int>(pos.y + size.y / 2.0f);

							island_bounds.push_back(ltrbi(l, t, r, b));
						}
					}
				}
			);
			...
			if (island_bounds.empty()) {
				/*
					Fall back: iterate all PHYSICAL sprites and compute a single bounding box.
				*/

				ltrbi total_aabb;
				bool first = true;

				project.nodes.template get_pool_for<editor_sprite_node>().for_each_id_and_object(
					[&](const auto&, const editor_sprite_node& node) {
						if (!node.active) {
							return;
						}

						if (const auto res = find_resource(node.resource_id)) {
							if (res->editable.domain == editor_sprite_domain::PHYSICAL) {
								const auto pos = node.editable.pos;
								const auto size = node.editable.size.has_value() ? 
									*node.editable.size : res->editable.size;
			...
			/*
				For each island, create a navmesh grid and mark occupied cells.
			*/

			auto& physics = scene.world.get_solvable_inferred().physics;
			const auto& b2w = physics.get_b2world();
			const auto pathfinding_filter = predefined_queries::pathfinding();
			const auto si = scene.world.get_si();
				...
				*/
				...
				for (const b2Body* body = b2w.GetBodyList(); body != nullptr; body = body->GetNext()) {
					for (const b2Fixture* fixture = body->GetFixtureList(); fixture != nullptr; fixture = fixture->GetNext()) {
						...
						/*
							Get the fixture's AABB and align it to the grid.
						*/
				...
				new_navmesh.islands.push_back(std::move(island));
			}
