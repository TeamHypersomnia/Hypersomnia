...
bool basic_physics_synchronizer<C>::is_constructed() const {
	return handle.get_cosmos().inferential.template get<physics_system>().is_inferred_state_created_for_rigid_body(handle);
}

template<bool C>
maybe_const_ref_t<C, rigid_body_cache>& basic_physics_synchronizer<C>::get_cache() const {
	return handle.get_cosmos().inferential.template get<physics_system>().get_rigid_body_cache(handle);
}
