...
public:
	...
	auto get_fixture_entities() const {
		return handle.get_cosmos().inferential.relational.fixtures_of_bodies.get_children_of(handle);
	}

	auto get_attached_joints() const {
		return handle.get_cosmos().inferential.relational.joints_of_bodies.get_all_children_of(handle);
	}
