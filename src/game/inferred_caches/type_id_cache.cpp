...
#include "type_id_cache.h"
#include "game/components/type_component.h"
#include "game/transcendental/entity_handle.h"
#include "game/transcendental/cosmos.h"

void type_id_cache::infer_cache_for(const entity_id id, const components::type& name) {
	entities_by_type_id[name.type_id].emplace(id);
}

void type_id_cache::destroy_cache_of(const entity_id id, const components::type& name) {
...
void type_id_cache::infer_cache_for(const const_entity_handle h) {
	infer_cache_for(h, h.get<components::type>().get_raw_component());
}

void type_id_cache::destroy_cache_of(const const_entity_handle h) {
	destroy_cache_of(h, h.get<components::type>().get_raw_component());
}

std::unordered_set<entity_id> type_id_cache::get_entities_by_type_id(const entity_type_id id) const {
