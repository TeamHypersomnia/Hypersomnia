	...
	cosm.for_each_having<components::trace>(
		[&](const auto t) {
			...
			if (trace.is_it_a_finishing_trace) {
				...
				/*
					In-flight lengthening is disabled - the line trails from make_line_trail
					have replaced it. The sprite stays at its base size; traces only shrink
					once they become finishing traces upon impact.
				*/
				trace.last_size_mult = trace_def.additional_multiplier;
				trace.last_center_offset_mult = vec2::zero;
