...
void visibility_system::respond_to_visibility_information_requests(
	...
	const auto& settings = cosmos.significant.meta.global.visibility;
	...
	const physics_system& physics = cosmos.inferential.get<physics_system>();
			...
			[&](const b2Fixture* const f) {
				...
				if (shape->GetType() == b2Shape::e_polygon) {
					...
					for (int32 vp = 0; vp < poly->GetVertexCount(); ++vp) {
						...
						push_vertex(vec2(vv).rotate_radians(rotation, vec2(0, 0)) + vec2(position), true);
					}
				}
