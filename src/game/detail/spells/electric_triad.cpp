...
void electric_triad_instance::perform_logic(const spell_logic_input in) {
	const auto& spell_data = std::get<electric_triad>(in.subject.get_cosmos().get_global_state().spells);
	...
	for (unsigned i = 0; i < 3 && i < static_cast<unsigned>(hostiles.size()); ++i) {
		...
		new_energy_ball.get<components::all_inferred_state>().set_activated(true);
	}
}