...
/*
	...
	2 - shadows of a light with a height, into its mask (min blending).

	Then:

	4 - the light itself over its whole reach, premultiplied, to be scaled by its mask,
	7 - the light removed by its shadows, into the removed light texture, reading the mask from light_mask_texture.
*/
...
{	
	...
	if (light_pass == 7) {
		...
		outputColor = vec4(premultiplied * (1.0 - mask), 1.0);
		return;
	}
