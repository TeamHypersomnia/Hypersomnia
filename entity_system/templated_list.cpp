...
namespace augmentations {
	namespace entity_system {
		void get_types(templated_list<>, const std::vector<base_type>&) {}

		void type_pack::add(const type_pack& adder) {
			/* just concatenate two vectors */
			for(auto it = adder.raw_types.begin(); it != adder.raw_types.end(); ++it)
				raw_types.push_back(*it);
		}

		void type_pack::remove(const type_pack& remover) {
			...
			/* delete every system found on remover's list */
