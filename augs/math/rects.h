...
namespace augs {
	...
	namespace rects {
		...
		struct ltrb {
			...
			ltrb() : l(static_cast<T>(0)), t(static_cast<T>(0)), r(static_cast<T>(0)), b(static_cast<T>(0)) {}
			...
			ltrb(const T l, const T t, const T r, const T b) : l(l), t(t), r(r), b(b) {}
			ltrb(const vec2t<T> pos, const vec2t<T> size) : l(pos.x), t(pos.y), r(pos.x + size.x), b(pos.y + size.y) {}
			...
			vec2t<T> get_sticking_offset(const sticking mode) const {
				vec2t<T> res;
				switch (mode) {
		...
		struct xywh {
			...
			xywh() : x(static_cast<T>(0)), y(static_cast<T>(0)), w(static_cast<T>(0)), h(static_cast<T>(0)) {}
			xywh(const ltrb<T> rc) : x(rc.l), y(rc.t) { b(rc.b); r(rc.r); }
			xywh(const T x, const T y, const T w, const T h) : x(x), y(y), w(w), h(h) {}
			xywh(const T x, const T y, const vec2t<T>& s) : x(x), y(y), w(s.x), h(s.y) {}
			xywh(const vec2t<T>& p, const vec2t<T>& s) : x(p.x), y(p.y), w(s.x), h(s.y) {}
