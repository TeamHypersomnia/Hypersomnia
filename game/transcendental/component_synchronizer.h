...
template <class T>
auto& get_component_ref_from_synchronizer_or_component(T& s) {
	return s;
}

template <bool is_const, class component_type>
auto& get_component_ref_from_synchronizer_or_component(component_synchronizer<is_const, component_type> s) {
	return s.get_data();
}
