...
void immediate_hud::acquire_game_events(
	...
	auto rng = fast_randomization(static_cast<size_t>(current_time * 10000));

	erase_remove(exploding_rings, [&](messages::exploding_ring& r) {
		const auto secs_remaining = r.maximum_duration_seconds - (current_time - r.time_of_occurence);
