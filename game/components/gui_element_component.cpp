	...
	void gui_element::draw_cursor_and_tooltip(vertex_triangle_buffer& output_buffer, const viewing_gui_context& context) const {
		...
		if (!is_dragging) {
			...
			if (maybe_hovered_item) {
				...
				const auto hovered = cosmos[get_hovered_world_entity(cosmos, step.camera_state.camera.transform.pos + rect_world.last_state.mouse.pos - step.camera_state.camera.visible_world_area /2)];
