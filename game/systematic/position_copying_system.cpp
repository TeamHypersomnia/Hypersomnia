...
void position_copying_system::update_transforms(fixed_step& step) {
	...
	for (const auto& it : targets) {
		...
		if (position_copying.target_newly_set) {
			auto target_transform = position_copying.subscribe_to_previous ? cosmos[position_copying.target].get<components::transform>().previous : cosmos[position_copying.target].get<components::transform>();
