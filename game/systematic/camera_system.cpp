...
void camera_system::react_to_input_intents(fixed_step& step) {
	...
	for (auto it : events) {
		if (!cosmos[it.subject].has<components::camera>())
			continue;
...
void camera_system::resolve_cameras_transforms_and_smoothing(fixed_step& step) {
	...
	for (auto e : cosmos.get(processing_subjects::WITH_CAMERA)) {
		...
		if (camera.enable_smoothing) {
			...
			float averaging_constant = 1.0f - static_cast<float>(pow(camera.smoothing_average_factor, camera.averages_per_sec * delta.in_seconds()));
		...
		if (entity_to_chase.alive()) {
			...
			if (entity_to_chase.has<components::physics>()) {
				...
				target_value = (player_pos - camera.previous_seen_player_position) * delta.in_milliseconds();
