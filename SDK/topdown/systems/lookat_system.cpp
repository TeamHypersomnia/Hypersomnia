	...
	for (auto it : targets) {
		...
		if (lookat.look_mode == components::lookat::look_type::POSITION) {
			...
			if (target_transform != nullptr)
				transform.current.rotation = (target_transform->current.pos - transform.current.pos).get_degrees();
		}
			...
			if (target_physics != nullptr)
				transform.current.rotation = vec2<>(target_physics->body->GetLinearVelocity()).get_degrees();
		}
	}
}
