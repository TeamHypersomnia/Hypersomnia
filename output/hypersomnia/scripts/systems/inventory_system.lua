...
function inventory_system:holster_weapon(subject_inventory, wielder, item)
...
function inventory_system:select_weapon(subject_inventory, wielder, item)
		...
		if msg.state_flag then
			...
			elseif intent == custom_intents.HOLSTER_ITEM then
				if inventory.active_item then
					...
					self:holster_weapon(subject_inventory, wielder, item)
				...
				for j=1, #inventory.slots do
					if intent == custom_intents["SELECT_ITEM_" .. j] then
						...
						if item and inventory.active_item ~= j then
							...
							self:select_weapon(subject_inventory, wielder, item)
