...
namespace components {
	struct animate : public augs::entity_system::component {
		...
		void set_current_frame(unsigned number, augs::entity_system::entity*, bool do_callback = true);
		unsigned get_current_frame() const {
			return current_frame;
		}

		void increase_frame(augs::entity_system::entity* sub) {
			set_current_frame(current_frame + 1, sub);
		}

		void decrease_frame(augs::entity_system::entity* sub) {
			set_current_frame(current_frame - 1, sub);
		}

		void set_current_animation_set(resources::animate_info*, augs::entity_system::entity* subject);
	private:
