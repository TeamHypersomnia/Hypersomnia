...
void lookat_system::resolve_lookat_value(augs::entity_id it) {
...
void lookat_system::update_physical_motors() {
	for (auto it : targets) {
		auto& lookat = it->get<components::lookat>();

		if (lookat.update_value) {
			if (lookat.use_physical_motor) {
				resolve_lookat_value(it);

				auto& physics = it->get<components::physics>();
				physics.enable_angle_motor = true;
				physics.target_angle = lookat.last_value;
			}
		}
	}
}
...
void lookat_system::update_rotations() {
	for (auto it : targets) {
		...
		if (lookat.update_value) {
			if (!lookat.use_physical_motor) {
				resolve_lookat_value(it);
				it->get<components::transform>().rotation = lookat.last_value;
			}
		}
	}
}
